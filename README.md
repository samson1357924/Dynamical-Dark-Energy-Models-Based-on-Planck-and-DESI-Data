...
The primary analysis script (`A+B.py`) represents the integration of two critical numerical phases:

1.  **Phase A (Forward Evolution - `forward_evolution.py`)**: 
...
2.  **Phase B (Backward Evolution & Growth Analysis - `backward_growth.py`)**:
    ...
    - **Optimization Tool (`optimizer_dm1060.py`)**: This specialized script is used to find the most suitable $D_{m1060}$ value. It treats $D_{m1060}$ as an optimization parameter to ensure that the backward integration results in a growth factor $\theta_0 = 1$ at the present epoch.
...
3. **`optimizer_dm1060.py`**: Optimized search for the $D_{m1060}$ parameter to satisfy the $\theta_0 = 1$ condition.
